import { ContactUs } from '../@types/contactUs';

export const contactUs: ContactUs = {
  companyNews: '企业动向',
  corporateResponsibility: '企业责任',
  announcements: '平台公告',
  throughOurCompanyNews:
    '通过公司新闻更新，更进一步了解WCG的最新进展，如新产品推介、交易时间变更，以及促销活动等的消息。',
  contactUs: '与我们联系',
  ifYouNeed: '如果您需要即时的协助，请填写以下的表格以联系我们的客服专员。',
  customerService: '客户服务热线',
  customerServiceEmail: '客户服务电邮',
  WCGIsAlways:
    'WCG始终在您身边。如您有任何疑问、问题、建议或要求，请随时与我们联系。我们提供多语言支持服务，如果您需要即时的协助，请填写以下的表格以联系我们的客服专员。',
  firstName: '名字',
  lastName: '姓氏',
  contactNumber: '联系电话',
  emailAddress: '电邮地址',
  area: '地区',
  category: '类别',
  pleaseSelectAnItem: '请选择清单中的一项',
  generalInquiry: '一般查询',
  disputesComplaints: '纠纷/投诉',
  partner: '合作伙伴',
  yourAccount: '您的帐号',
  areYouOurExistingCustomer: '您是现有客户吗',
  Yes: '是',
  No: '否',
  leaveAMessage: '留言',
  iAgreeTo:
    '我同意将我的信息提交到WCG，对有关产品和其他优惠的信息，可通过电邮和电话与我沟通。我已阅读并同意WCG隐私政策。您可以随时选择拒绝接受我们的行销通信。',
  submit: '提交咨询单',
  weWillContactYou: '我们将尽快与您联系',
  tooShort: '内容过短',
  tooLong: '内容过长',
  invalidFormat: '格式错误',
  required: '必填',
  January: '一月',
  February: '二月',
  March: '三月',
  April: '四月',
  May: '五月',
  June: '六月',
  July: '七月',
  August: '八月',
  September: '九月',
  October: '十月',
  November: '十一月',
  December: '十二月',
  establishedHongKong: '成立香港晨辰贸易有限公司',
  wonTheNo: '荣获香港金银业贸易场12号行员牌照，主营贵金属的国际贸易业务',
  changedItsName: '更名为文传金业有限公司',
  theOnlineTradingPlatform: '正式推出线上交易平台，方便客户可以对现货进行对冲',
  theInternationalTradeVolume: '实体贵金属的国际贸易量达日均200多公斤以上',
  setUpVietnam: '设立越南联络处',
  invitedToParticipate: '受邀参加第十六届中国上海理财博览会，接受媒体专访',
  partneredWith:
    '伙拍印度尼西亚当地知名持牌交易商*–*中央资本期货有限公司，正式迈进全球第四大市场',
  awardedTheMostInvestorTrusted: '荣膺年度最具投资者信赖贵金属交易商',
  WCGMarketsLtdObtained:
    'WCG*Markets*Ltd取得圣文森特和格林纳丁斯金融服务管理局（SVGFSA）牌照，号码为126087*BC*2020',
  establishedMalaysia: '设立马来西亚联络处',
  setUpTaiwan: '设立台湾联络处',
  aLeaderInTheIndustry:
    '业界翘楚吴国强先生强势加盟，负责风控及监控部门。吴先生拥有财务硕士学位及多项国际专业资格，历任多家世界级银行首席交易员和风险管理部主管',
  WCGMarketsLtdObtainedTheFINTRAC:
    'WCG*Markets*Ltd取得拿大FINTRAC*MSB牌照，号码为*M20282836',
  startCharityProjects:
    '启动慈善项目，积极参与教育及医疗慈善项目，致力成为良好企业公民，回馈社会',
  integrateAndUpdate:
    '整合及更新品牌。采用全新标志和标语，也隆重推出全新网页，具备多国语言，便利国际投资者',
  theTradingSystem:
    '交易系统升级，推出更多新产品，如美股，加密货币等，优化客户的投资体验',
  focusingOn:
    '心系乡村素质教育，特赞助知行教育基金会*21-22*年度知行图书角项目，预计最少有6所小学，数以千计的小学生可因此获益'
};

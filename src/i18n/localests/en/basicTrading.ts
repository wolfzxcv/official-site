...
export const basicTrading: BasicTrading = {
  ...
  thisIsTheThirdTradingMethod:
    ...
    afterTheDownloadProcess:'After*the*download*process*is*completed,*start*the*installation*process:',
    checkYesIAgreeWith:
    'Check*"Yes,*I*agree*with*all*the*terms*of*this*license*agreement"*and*then*press*the*"Next"*button.*If*you*need*to*specify*the*installation*path*of*the*MT4*platform,*click*the*"Settings"*button.',
  theInstallationWizard:
    'The*installation*wizard*will*automatically*connect*to*the*MetaQuotes*website*and*start*downloading*platform*content.*When*the*download*progress*bar*is*complete,*click*the*"Finish"*button.',
  thePlatformInterface:
    'The*platform*interface*will*automatically*open,*indicating*that*the*MT4*platform*has*been*successfully*installed.*If*you*encounter*any*problems*during*the*installation*process,*please*contact*the*online*customer*service*or*call*the*toll-free*number*to*contact*the*customer*service*team.',
  logInToTheTradingAccount:
    'Log*in*to*the*trading*account.*If*you*already*have*a*trading*account,*please*click*"File"*on*the*top*left*of*the*platform-"Log*in*to*trading*account.',
  inThePopUpLoginWindow:
    'In*the*pop-up*login*window,*enter*the*account*number*and*password,*choose*the*correct*server,*and*then*click*the*"Login"*button.',
  ...
  createAMarketOrder: 'Create*a*market*order',
  thereAreManyWaysTo:
  ...
  productCategory: 'Product*category:*Choose*your*trading*targets',
  lotsEnterTheLotSize: 'Lots:*Enter*the*lot*size,*the*minimum*is*0.01*lots*(foreign*exchange,*gold*and*silver).',
  stopLossPrice: 'Stop*Loss*Price:*When*the*price*reaches*the*specified*price,*the*position*is*automatically*closed*out;*the*default*is*zero,*which*means*no*stop*loss*is*set.',
  takeProfitprice: 'Take*Profit*price:*When*the*price*reaches*the*specified*price,*the*position*will*be*automatically*closed*to*keep*the*profit;*the*default*is*zero,*which*means*no*take*profit*is*set.',
  ...
  theOrderTypeColumn: 'The*"Order*Type"*column*in*the*order*window,*the*default*selection*is*"Market*Price",*which*means*to*create*a*transaction*at*the*current*market*price.',
  pendingOrder: 'Pending*order',
  changeTheOrderType: 'Change*the*order*type*to*"Pending*Order"*in*the*order*window*to*execute*the*pending*order*instruction.',
  thereAreFourTypes: 'There*are*four*types*of*pending*orders:',
  buyLimitBuyPending: 'Buy*limit——Buy*pending*orders*at*price*which*is*lower*than*the*market*price;',
  sellLimitSellPending: 'Sell*limit——Sell*pending*orders*at*price*which*is*higher*than*the*market*price;',
  buyStopBuyPending: 'Buy*stop——Buy*pending*orders*at*price*which*is*higher*than*the*market*price;',
  sellStopSellPending: 'Sell*stop——Sell*pending*order*at*price*which*is*lower*than*the*market*price.',
  afterSelectingTheCorresponding: 'After*selecting*the*corresponding*pending*order*type,*enter*the*required*price*in*the*price*column*and*click*"Place*Order".',
  oneClickTrading: 'One-click*trading',
  foreignExchangePrices: 'Foreign*exchange*prices*fluctuate*rapidly,*sometimes*we*have*to*find*a*faster*and*more*efficient*way*to*place*orders.*Fortunately,*MT4*provides*one-click*trading*functions.*Right*click*with*your*mouse*in*the*chart*window*on*the*right*side*of*the*platform*and*select*"One*Click*Trading"*in*the*menu*options.',
  onTheUpperLeftCorner: 'On*the*upper*left*corner*of*the*chart,*there*will*be*a*one-click*trading*quote*floating*window.*Left*click*on*the*price*to*automatically*open*a*sell*position,*and*then*right*click*on*the*price*to*automatically*open*a*buy*position.*Before*clicking,*be*sure*to*enter*the*number*of*lots*to*be*traded*in*the*middle*trading*volume*column.',
  modifyTheOrder: 'After*opening*a*position,*sometimes*we*need*to*set*a*stop*loss/profit*for*the*position.*In*the*"Trade"*column*of*the*"Summary"*window*at*the*bottom*of*the*platform,*find*the*position*that*needs*to*be*modified,*right-click*on*it,*and*select*"Modify*or*Delete*Order"*from*the*menu.',
  inThePopUpWindow: 'In*the*pop-up*window,*follow*the*settings*in*the*figure*below*to*change*the*stop*loss/take*profit*of*the*position.',
  closePosition: 'Close*position',
  theSummaryWindowAt: 'the*"Summary"*window*at*the*bottom*of*the*platform:',
  theAccountBalance: 'The*account*balance,*net*value,*used*margin*(occupied*margin),*and*available*margin*are*also*displayed*in*the*window*and*used*margin*ratio*(used*margin*ratio*=*equity/used*margin).*Now*we*close*this*transaction,*right-click*on*the*order*and*select*"Close*Position".',
  atThisTimeWeOpened: 'At*this*time,*we*opened*the*closed*trade*confirmation*window*and*clicked*on*the*yellow*button*in*the*window*to*close*this*trade*and*realize*profit*for*the*position.',
  oneClickClosing: 'One-click*closing:*Click*the*X*button*on*the*far*right*hand*side*of*the*position*to*activate*one-click*trading,*which*can*complete*the*closing*operation*more*quickly.',
  accountHistory: 'Account*history',
  youCanFindAllClosed: 'You*can*find*all*closed*positions*in*the*"Account*History"*column*of*the*"Summary"*window.',
  profitLossTheTotal: 'Profit/Loss:*the*total*profit*and*loss*of*the*transaction,*including*rollover*interest.',
  depositsTotalDeposits: 'Deposits:*total*deposits.',
  withdrawalTheTotal: 'Withdrawal:*the*total*amount*of*withdrawals.',
  openTheChartAndLoad: 'Open*the*chart*and*load*the*technical*indicators',
  afterTheMT4Platform: 'After*the*MT4*platform*is*installed,*four*charts*will*be*opened*by*default.*If*you*want*to*create*a*new*chart,*the*easiest*way*is*to*right-click*on*the*price*quotation*of*the*product*that*needs*the*chart,*and*select*the*menu*option*"Chart*Window".',
  ifYouWantToSwitch: 'If*you*want*to*switch*the*time*period*of*the*current*chart,*you*can*find*it*directly*on*the*toolbar*at*the*top*of*the*platform.*The*list*of*technical*indicators*that*comes',
  withMT4IsLocatedInThe: 'with*MT4*is*located*in*the*navigation*window*on*the*left*side*of*the*platform.*Right-click*the*indicator*that*needs*to*be*added*to*the*chart*and*select*"Attach*to*Chart"*to*add*the*indicator*to*the*current*chart.',
  afterReadingThis: 'After*reading*this,*you*have*learned*the*basic*operations*that*you*must*master*when*using*the*MT4*platform.*It*is*recommended*that*you*register*for*a*demo*account*and*practice*harder.*If*you*have*any*questions*about*the*platform,*you*are*welcome*to*contact*WCG’s*online*customer*service.',
  leverageAndMarginMargin: 'Leverage*and*margin*(margin*is*also*called*"prepayment"*on*the*MT4*platform)*are*like*two*sides*of*a*coin，*they*actually*refer*to*the*same*concept.*Here*we*use*an*example*to*illustrate*how*leverage*and*margin*play*a*role*in*trading.',
  assumingThatA1Lot: 'Assuming*that*a*1*lot*USD/JPY*position*is*established，*the*value*of*the*position*is*100，000*USD:',
  whenTheLeverageRatioIs11: '->*When*the*leverage*ratio*is*1:1，*the*margin*occupied*by*the*position*is*USD*100，000.*In*other*words，*you*need*to*prepare*at*least*$100，000*to*open*an*equal*position*(the*stock*market*is*a*market*with*a*leverage*ratio*of*1:1).',
  whenTheLeverageRatioIs1400: '=>*When*the*leverage*ratio*is*1:400，*the*margin*occupied*by*the*position*is*100，000*/*400*=*USD*250.*In*other*words，*with*a*leverage*of*400*times，*you*can*open*a*position*worth*$100，000*with*just*$250*as*a*margin.',
  pleaseKeepInMindThat: 'Please*keep*in*mind*that*the*margin*is*the*deposit*that*needs*to*be*paid*to*establish*a*new*transaction，*it*is*not*a*charge*to*you.*When*a*transaction*is*opened，*a*part*of*the*funds*in*your*account*will*be*used*as*margin.*The*bigger*the*leverage，*the*smaller*the*margin*is*required*for*the*transaction.*When*the*trade*is*closed，*the*occupied*margin*will*become*available*margin*again*for*your*account*to*open*new*positions*or*maintain*the*floating*losses*of*other*positions.',
  weUsuallyRecommendTraders: 'We*usually*recommend*traders*to*use*leverage*with*caution.*Leverage*is*a*double-edged*sword.*It*can*amplify*profits*as*well*as*losses.*In*the*above*example，*assuming*that*there*are*100，000*US*dollars*in*the*account*and*the*leverage*ratio*is*1:1，*you*can*only*open*1*lot*of*USD/JPY.*When*the*leverage*ratio*is*1:400，*you*can*open*up*to*400*lots*of*USD/JPY.*JPY*(100，000*/*250)，*as*the*transaction*volume*increases，*the*potential*risks*and*profits*are*enlarged.',
  stopLossPriceWhenThePrice: 'Stop*Loss*Price:*When*the*price*reaches*the*specified*price，*the*position*is*automatically*closed*out;*the*default*is*zero，*which*means*no*stop*loss*is*set.',
  whenUsingLeverage: 'When*using*leverage，*you*also*need*to*prevent*the*risk*of*liquidation:*if*the*opened*position*is*too*large，*and*the*price*continues*to*fluctuate*in*the*unfavorable*direction*of*the*position，*when*the*account*equity*(account*capital*that*changes*with*the*profit*and*loss*of*the*position)*drops*to*50%*of*the*occupied*margin，*that*is，*when*the*prepayment*ratio*on*the*MT4*platform*drops*to*50%，*all*positions*in*the*account*will*be*forced*to*liquidate*immediately',
  weRecommendThatYou: 'We*recommend*that*you，*as*a*newbie，*try*different*leverages*in*your*demo*account*to*find*out*the*most*suitable*leverage*ratio*for*yourself.',
  contractForDifference: 'Contract*for*difference*"CFD"*as*a*derivative*investment*tool*allows*investors*to*trade*the*price*changes*of*various*financial*assets*such*as*stock*indexes*and*commodity*futures.*By*trading*CFDs，*investors*can*conveniently*speculate*in*different*financial*markets*without*actually*holding*the*underlying*assets*of*the*relevant*market.*Because*it*can*easily*meet*the*needs*of*diversified*investment*and*global*investment，*CFD*trading*has*become*more*and*more*popular.',
  exploreTheCfdMarket: 'Explore*the*CFD*market',
  index: '-Index',
  energy: '-Energy',
  preciousMetals: '-Precious*Metals',
  contractDetails: '-Contract*Details',
  WhatAreTheDifferentTypes: 'What*are*the*different*types*of*underlying*assets*of*CFDs?',
  aStockIndexAsAFinancial: 'A*stock*index*is*a*financial*asset*that*represents*the*value*of*a*specific*part*of*the*stock*in*the*stock*market.*For*example，*one*of*the*most*popular*stock*indexes，*the*Standard*&*Poors*500，*represents*the*overall*performance*of*the*leading*500*companies*publicly*traded*on*the*US*stock*market.*If*you*trade*the*S&P*500*CFDs*through*WCG，*you*can*decide*whether*to*open*a*buy*or*sell*position*by*speculating*whether*the*index*will*rise*or*fall.*WCG*also*provides*CFD*trading*on*other*famous*stock*indexes，*including*the*UK*FTSE*Index*(UK100)，*the*US*Dow*Jones*30*Index*(US30)，*the*US*Nasdaq*100*Index*(NAS100)，*the*French*CAC*40*Index*(FRA40)，*and*Europe*Stoxx*50*Index*(ESTX50)，*German*DAX*Index*(GER30)，*Nikkei*225*Index*(JPN225)*and*Australia*ASX*200*Index*(AUS200).',
  commodityFuturesAreAlso: 'Commodity*futures*are*also*a*kind*of*financial*asset.*It*represents*an*agreement*to*buy*or*sell*a*said*amount*of*a*certain*commodity，*such*as*platinum，*copper*or*crude*oil*at*a*predetermined*date*and*price.*Since*the*prices*of*commodities*always*fluctuate*with*changes*in*supply*and*demand，*traders*can*speculate*on*their*price*fluctuations*by*buying*and*selling*commodity*futures.*If*you*trade*platinum*CFDs*through*WCG，*you*can*predict*whether*the*price*of*platinum*will*rise*or*fall*in*the*future*and*decide*whether*to*open*a*buy*or*sell*order.*The*commodity*CFDs*provided*by*WCG*also*include*copper*and*crude*oil*(Texas*light*crude*oil*WTI*and*BRENT*crude*oil).',
  byOpeningAnAccountThrough: 'By*opening*an*account*through*WCG，*you*can*trade*foreign*exchange*and*stock*index*or*commodity*CFDs*on*the*same*platform*and*account.*You*will*be*able*to*freely*swift*between*the*foreign*exchange*market*and*different*CFD*markets，*open*and*close*positions*accurately.',
  whatAreTheAdvantages: 'What*are*the*advantages*of*CFD*trading？',
  theMostSignificantAdvantage: 'The*most*significant*advantage*of*trading*CFDs*is*that*you*can*use*up*to*100*times*leverage，*which*means*you*can*open*larger*transactions*with*lower*starting*capital*and*relatively*small*transaction*costs.*It*should*be*noted*that*although*leverage*can*magnify*profits，*it*will*also*magnify*losses.*Therefore，*you*need*to*adopt*appropriate*risk*management*strategies*when*trading*CFDs*with*leverage.*CFDs*also*support*two-ways*trading，*whether*it*is*a*bear*market*or*a*bull*market，*you*can*buy*and*sell*freely，*and*the*whole*process*you*will*not*be*charged*any*commission*or*financing*fees.*Trade*all*products*on*one*account*and*take*full*advantage*of*the*multiple*trading*opportunities*that*WCG*provides*you.'

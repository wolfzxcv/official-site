export type Platform = {
  /********
   * platform
   ********/
  /** Efficient and free trading */
  efficientAndFreeTrading: string;
  /** Personal trading indicators */
  personalTradingIndicators: string;
  /** MT4 provides simple and easy-to-use */
  MT4ProvidesSimpleAndEasyToUse: string;
  /** Advanced chart tools */
  advancedChartTools: string;
  /** MT4 provides 31 chart */
  MT4Provides31Chart: string;
  /** Diversified investment transactions */
  diversifiedInvestmentTransactions: string;
  /** The best way to manage risk */
  theBestWayToManageRisk: string;
  /** Intuitive and easy to use */
  intuitiveAndEasyToUse: string;
  /** Complete and comprehensive reports */
  completeAndComprehensiveReports: string;
  /** Automatic adaptability and scalability */
  automaticAdaptabilityAndScalability: string;
  /** Through the further development */
  throughTheFurtherDevelopment: string;
  /** Download WCG MT4 trading platform */
  downloadWCGMT4TradingPlatform: string;
  /** Leading trading and network technology */
  leadingTradingAndNetworkTechnology: string;
  /** for PC */
  forPC: string;
  /** for Mac */
  forMac: string;
  /** multi-account management system */
  multiAccountManagementSystem: string;
  /** for iPhone */
  forIPhone: string;
  /** for Android */
  forAndroid: string;
  /** MT4 pc version */
  MT4PcVersion: string;
  /** MetaTrader 4 is the most popular */
  MetaTrader4IsTheMostPopular: string;
  /** download immediately */
  downloadImmediately: string;
  /** Comprehensive technical analysis function */
  comprehensiveTechnicalAnalysisFunction: string;
  /** Professional icon functions */
  professionalIconFunctions: string;
  /** Automatic charting records */
  automaticChartingRecords: string;
  /** Self-defined technical indicators */
  selfDefinedTechnicalIndicators: string;
  /** Support multiple windows */
  supportMultipleWindows: string;
  /** Simple charting operation */
  simpleChartingOperation: string;
  /** Nine charting time periods */
  nineChartingTimePeriods: string;
  /** Why choose  */
  whyChoose: string;
  /** MT4 is the most popular foreign */
  MT4IsTheMostPopularForeign: string;
  /** It has won various awards  */
  itHasWonVariousAwards: string;
  /** more than 50 preset  */
  moreThan50Preset: string;
  /** A number of trading products  */
  aNumberOfTradingProducts: string;
  /** Compatible with Expert Advisors  */
  compatibleWithExpertAdvisors: string;
  /** It has a mobile version  */
  itHasAMobileVersion: string;
  /** Provide an advanced  */
  provideAnAdvanced: string;
  /** Download the WCG MT4  */
  downloadTheWCGMT4: string;
  /** Using MT4 allows you  */
  usingMT4AllowsYou: string;
  /** Trading system  */
  tradingSystem: string;
  /** Includes current price execution  */
  includesCurrentPriceExecution: string;
  /** Technical analysis  */
  technicalAnalysis: string;
  /** MT4 provides a more comprehensive  */
  MT4ProvidesAMoreComprehensive: string;
  /** Safe and convenient  */
  safeAndConvenient: string;
  /** Advanced trading and analysis functions  */
  advancedTradingAndAnalysisFunctions: string;
};

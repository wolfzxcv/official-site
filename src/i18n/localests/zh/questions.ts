import { Questions } from '../@types/questions';

export const questions: Questions = {
  questions: '常見問題',
  thisArticleCollects:
    '本文收集了大部分投資者經常會問到的問題，統一為您做個詳細的解答。',
  whatKindOfSupervision: 'WCG是什麽監管?',
  WCGIsABusinessBrand: 'WCG 是由多家公司共同使用的業務品牌，包括',
  WCGMarketsLtdIsSupervised:
    'WCG Markets Ltd 由加拿大金融情報機構 (FIU) 和加拿大金融交易和報告分析中心 (FINTRAC) 監管。 MSB 許可證編號為 M20282836。',
  WCGMarketsLtdIsRegistered:
    'WCG Markets Ltd 根據國際商法註冊並受聖文森特和格林納丁斯金融服務管理局 (SVGFSA) 監管，編號為 26087 BC 2020。',
  HowToOpenAnAccountInWCG: '如何在WCG開戶?',
  youCanRegisterYourRealAccount:
    '您可以在WCG官網注冊您的真實帳戶，或點此注冊。',
  WCGAccountOpeningAge: 'WCG開戶年齡',
  theAgeToOpenAnAccount:
    '您好，於WCG開戶年齡必須年滿18歲或以上，不接受65歲或以上歲數客戶申請開戶。',
  whatKindOfAccounts: '平台提供什麽種類的帳戶?',
  weProvideDifferentAccountTypes: '詳情請洽客服人員。',
  whatKindOfDocuments: '我需要上傳哪些檔案?',
  customersInMainlandChina:
    '中國大陸地區客戶僅需要上傳您的身份證明檔案；中國大陸地區以外客戶需要請您提供您的護照以及地址證明（如駕駛證、常住人口登記卡或近三個月水電煤帳單等）。',
  howLongWillItTake: '帳戶需要多長時間稽核完成?',
  accountOpeningWillTake: '開戶正常1個工作日內會辦理完成。',
  howLongWillItTakeToProcess: '辦理出金申請需要多長時間?',
  WCGProcessingWithdrawalApplicationTime:
    'WCG處理出金申請時間:中午11:00-12:00/ 下午2:00-5:00 每一小時處理一次出金，最快2小時內到帳，超過出金時間申請出金，會在下一個工作天進行處理。',
  WCGWithdrawalConditions: 'WCG出金條件?',
  ifTheCustomerDepositsFunds:
    '如客戶存入資金後，沒有進行任何交易就要提走，退回資金要收取3%手續費。',
  afterACertainTransactionVolume:
    '在滿足一定的交易量後，會豁免手續費。交易量的要求：以最近累計入金金額為準，例如最近一次入金金額為1000美元，需要交易1 手後再申請出金才免手續費，入金2000美金需要交易2 手，入金5000美金就需交易5手，以此類推。',
  ifTheTransactionVolume:
    '假如交易量不足，又堅持出金，需要以最近累計入金金額扣除已交易手數部分的3%作為出金手續費。 （客戶在出金時備註同意扣除手續費才會扣除，否則出金會被拒絕）。',
  ifTheTransactionVolumeIsInsufficient:
    '交易量不足，出金低於50美元需要支付5美元行政費用。',
  HowDoIKnowMyWithdrawal: '如何知道出金申請已經被準予?',
  onceTheWithdrawalApplication:
    '一旦提款申請獲得批準，出金金額將自動從平台中扣除，並減少可用於交易和維持任何未結頭寸的保證金。',
  whatIsTheMT4Platform: '什麽是MT4平台?',
  MetaTrader4:
    'MetaTrader 4（MT4）是由MetaQuotes軟件發展的一個交易平台，用於在線外匯交易、差價合約（CFD）和期貨市場。MT4平台可以通過網站登錄，也可以下載到您的電腦或任何智慧設備上。',
  howToDownload: '如何下載交易平台?',
  youCanClickHere: '您可以點此至官網下載頁面，選擇適合您的設備下載。',
  canITradeOverWeekends: '週末是否可以交易?',
  noTradingOverWeekends: '週末由於市場關閉，無法交易。'
};

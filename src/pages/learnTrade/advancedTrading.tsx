...
const advancedTrading: React.FC<{}> = () => {
  ...
  function heightCalculation(){
    if (document.getElementById("heightDetect3") && document.querySelector("body")){
      const bodyTop = document.querySelector("body").getBoundingClientRect().top
      const divTop = document.getElementById("heightDetect3").getBoundingClientRect().top
  ...
  return (
    <Wrapper>
      <Flex id="heightDetect3" display="flex" py={{base: 10, xl: 20}} bgColor="gray.50" direction="column" align="center">
        ...
        {widthCheck && <Box display="flex" my={{base: 0, xl: 20}}>
          ...
          <Box w="65vw" mx={3}>
            {data[page].map((each, index) => (
              <>
                {each.type === "text" && (
                  <Box pb={3} key={"advanced" + page + index + each.key} textAlign={isArabic ? 'right' : 'left'}>
                    {each.key}
                  </Box>
                )}
                {each.type === "title" && (
                  <Box fontWeight="bold" pt={index === 0 ?0 :3} key={"advanced" + page + index + each.key} textAlign={isArabic ? 'right' : 'left'}>
                    {each.key}
                  </Box>
                )}
                {each.type === "image" && (
                  <Image py={3} key={"advanced" + page + index + each.key} src={each.key} />
                )}
              </>
            ))}
            <Box pb={7}/>
